...
FileNames = ["adlut.npy", "bigboss.npy", "classic.npy", "haru.npy", "natu.npy", "aki.npy", "huyu.npy", "pokemon.npy", "wa.npy"]
ClassNames = ["adlut", "bigboss", "classic", "haru", "natu", "aki", "huyu", "pokemon", "wa"]
hw = {"height":88, "width":200}
def PreProcess(var_amount=5):
    num = 0
    arrlist = []
    for dirname in ClassNames:
        files = glob.glob("img/{}/*.jpg".format(dirname))
        print(files)
        ...
        np.save('preprocess/'+dirname+".npy", nplist)

def create_png_model(ipshape=(32, 32, 3), num_classes=len(ClassNames)):
    model = Sequential()
...
def Learning(tsnum=30, nb_epoch=50, batch_size=8, learn_schedule=0.9):
    ...
    for filename in FileNames :
        data = np.load('preprocess/'+filename)          # 画像のnumpyデータを読み込み
    ...
    model = create_png_model(ipshape=(X_TRAIN.shape[1], X_TRAIN.shape[2], X_TRAIN.shape[3]), num_classes=target)
...
def TestProcess(imgname):
    ...
    i = random.randrange(0,stop=8,step=1)
    pred[0][i] = pred[0][i] + random.random()
    print(">> 計算結果↓\n" + str(pred))
